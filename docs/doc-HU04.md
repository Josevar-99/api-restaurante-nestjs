...
| Testing | Jest + `ts-jest` (cobertura integrada, `--coverage`) |
...
Los tests del service y del controller usan repositorios y servicios simulados (`jest.fn()`), por lo que no necesitan base de datos.

### 8.3 Reporte de cobertura (`jest --coverage`)
...
**Exclusiones del reporte** (`collectCoverageFrom` en la config de Jest, `package.json`): `src/categories/**`, `**/*.dto.ts`, `**/*.module.ts`, `src/main.ts`, `dist/**` y `test/**`. Los DTO se excluyen porque son clases declarativas y ya tienen su propia suite de validación.
